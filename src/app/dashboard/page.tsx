...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `API 요청 실패: ${errorData.error || response.statusText}`
        );
      }
  ...
  const storesWithMostPendingItems = useMemo(() => {
    ...
    dashboardData.unconfirmedByStore.forEach((ucs) => {
      const existing = pendingCounts.find(
        (pc) => pc.storeName === ucs.storeName
      );
      if (existing) {
        existing.pendingCount += ucs.count;
      } else {
        pendingCounts.push({
          storeName: ucs.storeName,
          pendingCount: ucs.count,
        });
      }
    });
    ...
    dashboardData.inProgressByStore.forEach((ips) => {
      const existing = pendingCounts.find(
        (pc) => pc.storeName === ips.storeName
      );
      if (existing) {
        existing.pendingCount += ips.count;
      } else {
        pendingCounts.push({
          storeName: ips.storeName,
          pendingCount: ips.count,
        });
      }
    });
  ...
  if (!dashboardData) {
    return (
      <div className="container mx-auto p-6 text-center">
        대시보드 데이터를 표시할 수 없습니다. API 응답을 확인해주세요.
      </div>
    );
  }
  ...
  const pieChartProgressData = progressStatusCounts.map((item) => ({
    name: item.name, // Assumes API provides translated name
    value: item.count,
  ...
  const pieChartDeliveryData = deliveryData.map((item) => ({
    ...
    fill:
      item.name === "직접배송"
        ? CHART_COLORS.DIRECT_DELIVERY
        : CHART_COLORS.COURIER_DELIVERY,
  }));
    ...
    <div className="container mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <Card className="shadow-md hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${KPI_COLORS.TOTAL}`}>
              {overallStats?.totalItems ?? 0}개
            </div>
        ...
        <Card className="shadow-md hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${KPI_COLORS.UNCONFIRMED}`}>
              {overallStats?.totalUnconfirmed ?? 0}개
            </div>
            <p className="text-xs text-gray-500 pt-1">
              확인/처리 대기 중인 비품
            </p>
        ...
        <Card className="shadow-md hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${KPI_COLORS.IN_PROGRESS}`}>
              {overallStats?.totalInProgress ?? 0}개
            </div>
            <p className="text-xs text-gray-500 pt-1">
              현재 처리/배송 진행 중인 비품
            </p>
        ...
        <Card className="shadow-md hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${KPI_COLORS.COMPLETED}`}>
              {overallStats?.totalCompleted ?? 0}개
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <Card className="shadow-md">
          <CardHeader>
            ...
            {/* Assuming re-typing fixed the unescaped entity error here */}
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <Tooltip
                  formatter={(value: number) => [`${value}개`, "총 수량"]}
                />
                <Bar
                  dataKey="totalQuantity"
                  fill={CHART_COLORS.STORE_QUANTITY}
                  radius={[4, 4, 0, 0]}
                  barSize={40}
                />
              </BarChart>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <Card className="shadow-md">
          <CardHeader>
            ...
            <CardDescription>
              주의가 필요한 &apos;미확인&apos; 상태의 품목이 많은 매장
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                data={unconfirmedByStore}
                margin={{ top: 5, right: 20, left: -10, bottom: 5 }}
              >
                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey="storeName" tick={{ fontSize: 12 }} />
                <YAxis allowDecimals={false} tick={{ fontSize: 12 }} />
                <Tooltip
                  formatter={(value: number) => [`${value}개`, "미확인 수량"]}
                />
                <Bar
                  dataKey="count"
                  fill={CHART_COLORS.UNCONFIRMED}
                  radius={[4, 4, 0, 0]}
                  barSize={40}
                />
              </BarChart>
        ...
        <Card className="shadow-md">
          <CardHeader>
            ...
            <CardDescription>
              현재 활발하게 처리 중인 품목이 많은 매장
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                data={inProgressByStore}
                margin={{ top: 5, right: 20, left: -10, bottom: 5 }}
              >
                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey="storeName" tick={{ fontSize: 12 }} />
                <YAxis allowDecimals={false} tick={{ fontSize: 12 }} />
                <Tooltip
                  formatter={(value: number) => [`${value}개`, "진행 중 수량"]}
                />
                <Bar
                  dataKey="count"
